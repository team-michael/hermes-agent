    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # Workers AI code 3040 is transient provider capacity, not an
                # account quota. Let the configured retry budget and jittered
                # backoff run before falling through to the fallback chain.
                _is_cloudflare_capacity = is_cloudflare_workers_ai_capacity_error(
                    base_url=str(_base), error=api_error
                )
                _should_fallback = (
                    (is_rate_limited and not _is_cloudflare_capacity)
