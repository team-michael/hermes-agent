...
def _extract_text_from_slack_attachment_blocks(
    blocks: list, max_chars: int = 6000
) -> str:
    """Extract evidence-bearing text from attachment Block Kit.

    Amazon Q / AWS Chatbot sends alarm details in ``attachments[].blocks[]``.
    Action blocks are intentionally ignored because their callback IDs and
    values are executable payloads rather than user-visible evidence.
    """
    lines: list[str] = []
    seen: set[str] = set()
    ...
    def _clean(value: Any) -> str:
        ...
        if not isinstance(value, str):
            return ""
        # Slack mrkdwn links use <url|label>; the readable label contains the
        # alarm name while the URL is noisy and can confuse alarm-name parsing.
    ...
    for block in blocks or []:
        ...
        if block_type in {"section", "header"}:
            ...
            _append(_extract_text_from_slack_blocks([block]))
...
class SlackAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Per-message subscription metadata for conditional final reactions.
    ...
    def _inline_command_name(text: str) -> str:
        """Return the inline slash/bang command name from a Slack message."""
    ...
    def _extract_slack_table_from_content(
        self, content: str
    ) -> tuple[str, Optional[dict], Optional[str]]:
        """Extract one profile-gated ``slack-table`` response directive."""
        ...
        try:
            ...
            logger.debug(
                "[Slack] slack-table directive unavailable: %s", exc, exc_info=True
            )
            return content, None, None
        ...
        try:
            table = json.loads(match.group("json").strip())
        except Exception as exc:
            return cleaned, None, f"Invalid slack-table JSON: {exc}"
        ok, error = _validate_slack_table(table)
        if not ok:
            return cleaned, None, f"Invalid slack-table payload: {error}"
        try:
            return cleaned or "(table)", _build_slack_table_block(table), None
        except Exception as exc:
    ...
    async def send(
        ...
        try:
            ...
            content, table_block, table_error = self._extract_slack_table_from_content(
                content
            )
            if table_error:
                suffix = f"> Slack table render failed: {table_error}"
            ...
            blocks = (
                ...
                if table_block is None and len(chunks) == 1
                else None
            )
    ...
    async def on_processing_complete(
        ...
        subscription_cfg = self._subscription_reaction_configs.pop(
            self._slack_reaction_key(channel_id, ts), None
        )
        if team_id:
        ...
        if outcome == ProcessingOutcome.SUCCESS:
            ...
            if subscription_cfg:
                ...
                if policy in {"resolved_marker", "marker"}:
                    if not self._slack_response_has_resolved_processing_status(
                        response_text
                    ):
                        return
                elif policy in {"processing_status", "status", "status_marker"}:
    ...
    @staticmethod
    def _slack_config_values(raw: Any) -> set[str]:
        if raw is None:
            return set()
    ...
    def _slack_message_subscriptions(self) -> list[Dict[str, Any]]:
        ...
        if isinstance(raw, str):
            ...
            try:
                ...
                logger.warning(
                    "[Slack] Ignoring invalid Slack message subscription JSON"
                )
                return []
        if isinstance(raw, dict):
            raw = [raw]
    ...
    def _matching_slack_message_subscription(
        self, event: dict
    ) -> Optional[Dict[str, Any]]:
        ...
        for subscription in self._slack_message_subscriptions():
            ...
            if not channels or identity["channel"] not in channels:
                continue
            subtypes = self._slack_config_values(
                subscription.get("subtypes") or subscription.get("subtype")
            )
    ...
    @staticmethod
    def _slack_subscription_bypasses_mention(
    ...
    @staticmethod
    def _slack_subscription_reactions_enabled(
        subscription: Optional[Dict[str, Any]],
    ) -> bool:
        if not subscription:
            return False
    ...
    @staticmethod
    def _slack_subscription_prompt(
        subscription: Optional[Dict[str, Any]],
    ) -> str:
        if not subscription:
            return ""
    ...
    def _prepend_slack_subscription_context(
        ...
        if subscription and subscription.get("name"):
            context.append(f"subscription: {subscription['name']}")
        return "\n".join(context) + "\n\n" + text
    ...
    @classmethod
    def _slack_response_has_resolved_processing_status(
        cls, response_text: str
    ) -> bool:
        return cls._slack_response_processing_status(response_text) in {
            "resolved",
            "no_action",
            "no-action",
            "false_positive",
    ...
    async def _handle_slack_message(
        ...
        if event.get("bot_id") or event.get("subtype") == "bot_message":
            early_team_id = self._event_team_id(event, payload)
            own_bot_uid = self._team_bot_user_ids.get(
                early_team_id, self._bot_user_id
            )
            bot_profile = event.get("bot_profile") or {}
            message_user = event.get("user") or bot_profile.get("user_id")
            if (own_bot_uid and message_user == own_bot_uid) or (
                event_ts and event_ts in self._bot_message_ts
            ):
                return
            ...
            if message_subscription:
                ...
                if own_bot_uid and f"<@{own_bot_uid}>" not in text_check:
                    return
            elif allow_bots == "all":
                pass
        ...
        if slack_attachments:
            ...
            for att in slack_attachments:
                ...
                blocks_text = _extract_text_from_slack_attachment_blocks(
                    att.get("blocks") or []
                )
                ...
                body = att_text or ("" if blocks_text else att_fallback) or ""
                if body:
                    body = body.strip()
                    if len(body) > 500:
                        body = body[:497] + "..."
                ...
                if blocks_text:
                    section = f"{section}\n{blocks_text}" if section else blocks_text
                ...
                if section in text or section in seen_sections:
                    continue
        ...
        inline_command_name = self._inline_command_name(original_text)
        inline_command_targets = self._inline_command_target_bot_ids(original_text)
        ...
        if (
            ...
            logger.debug(
                "[Slack] Ignoring message in muted thread %s:%s",
                channel_id,
                thread_ts,
            )
            return
        ...
        if is_mentioned:
            ...
            if text.startswith("!"):
                try:
                    ...
                    command_name = text[1:].split(maxsplit=1)[0].split("@", 1)[0]
                    if command_name and is_gateway_known_command(command_name):
                        text = "/" + text[1:]
                except Exception:  # pragma: no cover - defensive
                    pass
        ...
        if message_subscription:
            text = self._prepend_slack_subscription_context(
                text, event, message_subscription
            )
        ...
        subscription_prompt = self._slack_subscription_prompt(message_subscription)
        ...
        if _should_react:
            ...
            if message_subscription:
                self._subscription_reaction_configs[
                    self._slack_reaction_key(channel_id, ts)
                ] = message_subscription
    ...
    async def send_exec_approval(
        ...
        try:
            thread_ts = self._resolve_thread_ts(None, metadata)
            from tools.approval import get_approval_approver_users

            approvers = get_approval_approver_users("slack")
            owner_only = bool(approvers)
            ...
            if owner_only:
                mentions = " ".join(f"<@{user_id}>" for user_id in sorted(approvers))
                header += f"*Approval administrator:* {mentions}\n"
            if smart_denied:
            ...
            if not smart_denied and not owner_only:
                actions.append({
                    "type": "button",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        from tools.approval import can_user_resolve_approval

        if not can_user_resolve_approval("slack", user_id):
            logger.warning(
                "[Slack] Non-approver approval click by %s (%s) - ignoring",
                user_name,
                user_id,
            )
            return
    ...
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            if blocks:
                ...
                payload["text"] = formatted or "(attachment)"
...
def _apply_yaml_config(yaml_cfg: dict, slack_cfg: dict) -> dict | None:
    ...
    if "message_subscriptions" in slack_cfg and not os.getenv(
        "SLACK_MESSAGE_SUBSCRIPTIONS"
    ):
