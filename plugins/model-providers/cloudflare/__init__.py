...
def _normalize_reasoning_effort(reasoning_config: dict | None) -> str | None:
    ...
    GLM accepts the full Hermes ladder and Kimi accepts none/low/medium/high.
    Nemotron accepts low/medium/high; its model-specific ``none`` handling is
    applied by the provider profile after this shared normalization step.
    """
...
class CloudflareWorkersAIProfile(ProviderProfile):
    ...
    denominator for GLM, Kimi, and Nemotron on the OpenAI-compatible endpoint
    is the top-level ``reasoning_effort`` field.
    """
    ...
    def build_api_kwargs_extras(
        ...
        if effort and _is_cf_reasoning_family(model):
            if effort == "none" and _normalize_cf_model(model).startswith(
                "nvidia/nemotron-"
            ):
                return {}, {}
