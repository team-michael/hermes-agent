---
name: update-hermes
description: Update a Hermes checkout while preserving team-managed local customizations. Use when Codex needs to run `hermes update`, keep the primary checkout on `main`, rebase and push `local/hermes-patches` against `origin/main`, re-materialize `ignored/local`, and synchronize all repo-managed Hermes profiles, SOUL files, configs, shared files, and custom skills after an update.
---
...
6. Push `local/hermes-patches` to `team-michael/local/hermes-patches` with `--force-with-lease`.
7. Restore `ignored/local` from the rebased patch branch into the main checkout.
8. Run `ignored/local/scripts/apply-local-state.py --replace-existing --link-soul` with no profile arguments, so newly added repo-managed profiles are included automatically.
9. Recreate known shared symlinks under `~/.hermes/shared` and `~/.hermes/bin`.
10. Run `ignored/local/scripts/audit-local-state.py`.
...
- Keep local-only profile, SOUL, config overlay, shared, and custom skill assets under `ignored/local` unless upstream has a direct replacement.
