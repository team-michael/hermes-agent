...
§
Notifly GTM/outbound engineering v0 artifacts live under `/home/ubuntu/.hermes/profiles/sdr/workspace/gtm-copilot/`. Trackit Open API uses `NOTIFLY_TRACKIT_API_KEY` for object/record reads. `Notifly - Acquisition` is a Companies UI group/view (screenshot count 158); do not equate it with public API `braze` object count 184. Trackit group endpoints still require different auth/OAuth.