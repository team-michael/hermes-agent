...
§
E2E는 실데이터·canary 선호. 운영 집계는 캠페인×lang별 distinct 사용자와 이벤트 건수 구분. schema CI는 local PG+prod probe, tenant/global 독립.
§
...
§
설계/API는 저장 가능성부터 UI→transformer→runtime을 구분 검증. 다국어 발송 조사는 users_ locale·저장 localized message·delivery_result 렌더·message_events 도달을 분리 확인하길 원함.