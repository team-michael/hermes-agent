...
> **🚩 PRE-SEND CHECKLIST FOR ANY xdotool COMMAND** (this is a recurring chronic operator hazard — 5+ documented occurrences as of 2026-05-30):
>
> Before pressing send on a command containing `XAUTHORITY=`, scan the value character-by-character. The value MUST be the literal string `/home/ubuntu/.Xauthority`. Reject and retype the command if you see ANY of these:
> - `XAUTHORITY=***` (secret-redaction artifact carried over from a prior terminal echo)
> - `XAUTHORITY=/home/...rity` (tab-completion fragment — `...` is literal three dots, not a placeholder)
> - `XAUTHORITY=$XAUTHORITY` or `XAUTHORITY=$XAUTH` (env vars not inherited reliably through the terminal tool's bash -c wrapper)
> - omitted XAUTHORITY entirely while still passing `DISPLAY=:1`
>
> All four shapes have been observed to fail with the same `Authorization required, but no authorization protocol specified / Can't open display: (null)` cascade. Burning a single xdotool failure is cheap; burning four in a row while the user is waiting on the VNC handoff post is a credibility hit. **Type the full literal every time, even when the command echo looks tedious.**
...
> **⚠️ Correction to the 2026-05-26 claim that "Chrome started anyway / hermes_chrome.py resets XAUTHORITY internally" — this is WRONG for headful Selenium and caused a misdiagnosis on 2026-06-05.** `hermes_chrome.py` uses `os.environ.setdefault("XAUTHORITY", "/home/ubuntu/.Xauthority")` — `setdefault` only fills the var when it is UNSET. If you launch the scrape/verify script with a garbage `XAUTHORITY=***` or `XAUTHORITY=/home/...rity` in the terminal command, that bad value is already set in the child env, so `setdefault` is a no-op and the bad value STICKS. Headful Chrome then cannot reach the X server and exits immediately, surfacing as:
> ```
...
> ```
> **This is a NEW failure-mode-to-root-cause mapping.** The error is byte-identical to the documented "transient handshake race with ZERO live processes" (see SessionNotCreatedException pitfall below), so the instinct is to sweep singletons + single clean retry. That does NOT fix it — the retry re-passes the same bad `XAUTHORITY` and dies the same way. On 2026-06-05 this burned two retry cycles before the root cause (the `***` in my own launch command) was spotted.
>
> **Diagnostic to distinguish the two:** a quick `google-chrome --headless=new --user-data-dir=/tmp/x --dump-dom about:blank` will SUCCEED (headless needs no X), proving the Chrome binary is healthy and the problem is X-display reachability — i.e. a bad/missing `XAUTHORITY` in the headful launch env, not a singleton race.
>
> **Fix:** the cleanest is to pass NOTHING for XAUTHORITY in the launch command and let `hermes_chrome.py`'s `setdefault` apply the correct `/home/ubuntu/.Xauthority` default — i.e. launch with just `DISPLAY=:1 .../python script.py`. (Do NOT pass `XAUTHORITY=/home/...rity` "to be safe" — the tab-completion fragment is a non-existent path that defeats setdefault exactly like `***`.) This is the OPPOSITE of the xdotool rule (where you MUST pass the literal), because for Selenium launches `hermes_chrome.py` owns the default and you only break it by passing garbage. Rule of thumb: **xdotool → always pass the full literal; Selenium-script launch → pass DISPLAY only, omit XAUTHORITY.**

**Recurrence 2026-05-27 (third occurrence)**: hit it AGAIN despite the doubled warning above. Drafted four xdotool commands (`windowactivate`/`windowraise`/`windowsize`/`windowmove`) with `XAUTHORITY=***` placeholders — every call returned "Authorization required, but no authorization protocol specified / Can't open display: (null)". Re-ran with full literal `/home/ubuntu/.Xauthority` and all four succeeded immediately. **The placeholder slips in when copy-editing prior commands** that had the secret-redaction render of XAUTHORITY shown in the terminal echo — the redaction looks like a literal value at draft time. **Operating rule: before pressing send on any command containing `XAUTHORITY=`, scan the value. If it contains `*`, `...`, or anything other than a path starting with `/`, replace it with `/home/ubuntu/.Xauthority` literally.** This is the third documented occurrence (2026-05-13, 2026-05-26 ×2, 2026-05-27); treat it as a chronic operator hazard, not a one-time mistake.

**Recurrence 2026-05-29 (fourth occurrence) — and a correction to the "skip XAUTHORITY" advice**: hit the failure mode in a different shape today. First xdotool batch was drafted with `DISPLAY=:1` only (no XAUTHORITY, following the prior section's "cheapest prevention: skip XAUTHORITY entirely") — every call failed with "Authorization required, but no authorization protocol specified / Can't open display: (null)". Adding the env var back (even as the bogus `/home/...rity` placeholder) made all four xdotool commands succeed instantly. **So "skip XAUTHORITY" is NOT reliable advice on this host** — the agent's terminal tool wrapper does not inherit XAUTHORITY from the parent shell's session, so xdotool sees no authority cookie when the var is omitted. The Chrome wrapper has its own XAUTHORITY default and is unaffected, which is why omitting it sometimes appears to "work" — but xdotool is strict.

**Recurrence 2026-05-30 (fifth occurrence)** — the chronic operator hazard fired again on the very next day after the 5/29 update. Drafted xdotool window-promote commands with `XAUTHORITY=***` placeholder values; the literal `***` (carried over from a prior secret-redacted terminal echo) made it past intake and four consecutive xdotool calls failed with the same "Authorization required / Can't open display: (null)" cascade. Re-running with literal `/home/ubuntu/.Xauthority` succeeded immediately on every call. **Reading the 5/27 entry that explicitly says "scan the value before sending" did NOT prevent the fifth occurrence** — the operator skim-pattern is "this looks like the 5/27 case, env vars are documented, send" and the actual character-by-character value scan never happens. That's why the PRE-SEND CHECKLIST callout was added at the top of this section. If you're reading this and you've just drafted a command with `XAUTHORITY=` in it, STOP. Look at the value. If it isn't the literal string `/home/ubuntu/.Xauthority`, fix it before sending. The fact that this skill has a checklist for ONE specific env-var value is itself the hazard signal — past you didn't follow the rule, present you doesn't believe past you, future you will hit it again.

**Updated operating rule (supersedes prior "cheapest prevention"):** ALWAYS pass `XAUTHORITY=/home/ubuntu/.Xauthority` explicitly on every xdotool command. Do NOT trust session inheritance. The two failure modes that bite are now documented:
1. Wrong VALUE (placeholder like `***` or `/home/...rity`) — fails loudly with "Authorization required". Fix: literal `/home/ubuntu/.Xauthority`.
2. MISSING entirely (omitted var, relying on session inheritance) — fails the same way. Fix: include the var literal.

The placeholder form `/home/...rity` actually worked as a value in 2026-05-29 by accident (xdotool may have fallen back to a session default when the path didn't exist) — DO NOT rely on this. Always type the full literal.
