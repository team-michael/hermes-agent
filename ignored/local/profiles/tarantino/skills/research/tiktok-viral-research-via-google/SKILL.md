...
- **4-day-streak deterministic block signature → split the cron, don't retry**. If the same 23-UTC (or any other fixed) cron slot hits `/sorry/` at the same Q-index multiple days running with the same query list (observed 2026-05-08/09/10/11 on `hl=ko`, Q14 `"find friends college app"` every single day — 4 consecutive confirmations), stop treating it as noise. It's an IP+time-slot reputation pattern that will not self-heal. Fix is structural, not retries:
  ...
  **`SessionNotCreatedException` misdiagnosis on 6/05 — see headful-chrome-vnc skill.** Between the clean Google run and the DOM-verify, the dom_verify launch died twice with `SessionNotCreatedException: Chrome instance exited` (zero live Chrome, only stale singletons). I misread it as the documented "(b) transient handshake race" and burned two retry cycles sweeping singletons + retrying. Root cause was actually that I passed a garbage `XAUTHORITY=*** placeholder in my own launch command, and `hermes_chrome.py`'s `os.environ.setdefault` left the bad value in place → headful Chrome couldn't reach X → exited. Fix: launch Selenium scripts with `DISPLAY=:1` ONLY (omit XAUTHORITY, let setdefault apply the correct default). Full writeup added to `headful-chrome-vnc` under the XAUTHORITY pitfall section.
  ...
  **KEY OPERATIONAL SHIFT (as of 2026-06-06): the bottleneck has fully migrated from Google to the TikTok cookie layer.** The position-13/14 Google block self-reset on 6/05 and has stayed clean two days running — Google is no longer the daily pain point. The recurring daily blocker is now **Stage-2 TikTok rotation captcha on the FIRST DOM-verify candidate**, and it fires because nobody solves the overnight handoff, so the TikTok cookie layer compounds staleness (24h → 48h → never warm). This is a different failure than the documented "two-stage captcha on recovery runs" (which assumed Stage 1 was the Google block). Now Stage 1 passes clean and Stage 2 is the ONLY block. **Implication for the handoff post**: lead with the fact that *only TikTok needs solving* (Google is fine) and emphasize the compounding — "어제 핸드오프 미솔빙 → TikTok 쿠키 48h+ 묵음 → 매일 1번째 영상부터 캡차". A single same-day TikTok solve breaks the compounding; the cost of NOT solving is that every subsequent tick hits Stage-2 on candidate #1. The SERP was again heavily polluted (top-by-views `@eliemagic` 1M dorm-tour, `@collegebycohort` 66K Common-App admissions content) — "app" pulls in "Common App" + dorm-tour lifestyle; manual ICP curation down to genuine social-app handles was mandatory before DOM verify. Add `eliemagic`, `collegebycohort`, `mamboitalianx`, `char56355`, `jayyyllen`, `brianna_zhang`, `pratikvangal`, `stresslesscollegeapps`, `collegecounselmore`, `yourcollegeguru`, `joinedvisorly`, `ciitofficial` (college-admissions/dorm-tour pollution) to NA-college NOISE_HANDLES.
