...
## Pitfalls
- **`SessionNotCreatedException: Chrome instance exited`** on `create_driver()` usually means a previous Chrome process is still holding the Tarantino profile lock. Symptoms: scrape script crashes immediately, `pgrep -af chrome` shows live processes from a prior run. **Fix**:
  ```bash
  pkill -9 -f 'user-data-dir=/home/ubuntu/.hermes/profiles/tarantino/Tarantino' 2>/dev/null
  pkill -9 -f chromedriver 2>/dev/null
  sleep 3
  ```
  Then re-run. Do NOT run two scrape scripts concurrently against the same profile — they will deadlock on the `SingletonLock` file.
