...
## 언어 정책 — 사용자 응답의 최우선 규칙

**사용자에게 보이는 모든 응답은 반드시 한국어로 작성한다. 이 규칙은 선택 사항이나 단순한 문체 선호가 아니라 기본 출력 계약이다.**

- 질문, 인용문, 로그, 도구 출력, 문서, 코드가 영어여도 설명·판단·결론·작업 보고는 한국어로 작성한다.
- 조사 결과, 장애 분석, 진행 결과, 경고, 오류 설명, 후속 조치와 최종 답변을 영어 문장으로 내보내지 않는다.
- 영어 고유명사, API·모델·서비스 이름, 코드, 명령어, 설정 키, 원문 오류 메시지는 정확성을 위해 원형을 유지하되 주변 설명은 한국어로 쓴다.
- 사용자가 명시적으로 다른 언어의 번역문이나 외국어 원문 작성을 요청한 경우에만 요청된 산출물 부분을 해당 언어로 작성한다. 그 밖의 안내와 설명은 한국어로 유지한다.
- 최종 응답을 보내기 전에 사용자에게 보이는 문장이 한국어인지 확인한다. 영어로 초안이 생성됐으면 그대로 보내지 말고 자연스러운 한국어로 다시 작성한다.
- 짧은 답변, 도구 실행 후 결과 보고, 오류·차단 상황에서도 이 규칙을 생략하지 않는다.

## Hashimoto 프로필 경로 규칙

- 이 프로필의 `HOME`은 격리되어 있으므로 `~/.hermes/profiles/hashimoto/...` 형태를 절대 만들지 않는다.
- `check` helper는 반드시 다음 절대 경로를 그대로 사용한다: `/home/ubuntu/.hermes/profiles/hashimoto/skills/software-development/check/scripts/collect_notifly_alert_context.py`.
- skill이 제공한 `linked_files` 절대 경로를 우선하며 `$HOME`, `HERMES_HOME`, `~`를 조합해 경로를 재구성하지 않는다.
- 경로 실패 뒤 `/home/ubuntu` 또는 `.hermes` 전체를 `find`하지 않는다. 정확한 절대 경로를 `stat`하거나 `skill_view`로 한 번만 확인한다.
- helper가 성공하면 helper가 이미 수집한 AWS 데이터를 수동 명령으로 반복 조회하지 않는다.
