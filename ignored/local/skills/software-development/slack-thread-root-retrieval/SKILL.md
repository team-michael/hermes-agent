...
It loads `SLACK_BOT_TOKEN` from the active environment or the active Hermes profile's `.env`, fetches `conversations.replies`, extracts message text/attachments/blocks into a compact summary, and writes raw + summary JSON under the active profile's `slack_api_cache/` without printing the token.
...
### Ambient token from another profile

The helper prefers an already exported `SLACK_BOT_TOKEN` over the active profile's `.env`. In a multi-profile shell, that ambient token may belong to another workspace and return `channel_not_found` even though the active profile has the correct token. Retry without the ambient variable so the helper loads the active profile token:

```bash
env -u SLACK_BOT_TOKEN python ~/.hermes/profiles/<active-profile>/skills/software-development/slack-thread-root-retrieval/scripts/fetch_slack_thread.py <channel_id> <thread_ts>
```

Verify the token identity/workspace without printing the token.

## Practical notes

- In Notifly Slack work, treat Slack permalinks as structured Slack data, not ordinary web links. Browser login pages are **not** evidence that content is inaccessible. **Do not open the permalink in the browser as the first retrieval attempt.** First parse channel/ts, load `SLACK_BOT_TOKEN` from the active profile's `.env` if it is not already in the process environment, then call Slack Web API (`conversations.history` with a tight `oldest/latest` window for the exact permalink message; `conversations.replies` with `thread_ts` for the full thread). If the bot token is not available in the active process, use Hermes-local recovery within the active profile before giving up. Never inspect another profile's credentials, sessions, cache, or runtime files without explicit, narrowly scoped user authorization. Only say content is inaccessible after API-level failure **and** active-profile local/session-cache recovery fail; state those attempted paths explicitly. If the Slack message has `files[]`, download image evidence via `url_private_download` with the bearer token and run vision/OCR before summarizing. If the Slack content links to Google Docs/Sheets/Drive, follow with authenticated Google Workspace access before giving product/sales conclusions. See `references/slack-permalink-api-first-with-media.md` for the compact permalink + screenshot retrieval recipe.
- When a Hermes Slack DM is internally thread-scoped but appears in Slack Assistant `채팅/내역` instead of the app `메시지` tab, diagnose Slack app surface/manifest before changing session settings. Do **not** set `dm_top_level_threads_as_sessions: false` when the user wants per-thread DM sessions. Remove `assistant_view`, `assistant:write`, and Assistant events from the Slack manifest while keeping App Home `messages_tab_enabled`. See `references/slack-assistant-vs-messages-surface.md`. 
...
- In an active Hermes profile, Slack Web API access may require explicitly loading `SLACK_BOT_TOKEN` from that profile's `.env` inside a one-off script; terminal subprocesses may strip messaging secrets from ambient `os.environ`. Derive the profile directory from the executing skill/script path or accept it as an explicit argument. Never hard-code another profile's path and never print the token.
...
- When a user reports that Slack DM conversations are appearing as separate items under Slack Assistant “내역”/history instead of continuing in one chat, check Hermes session scoping before assuming a Slack retrieval failure. Inspect the active profile’s `sessions/sessions.json` and `channel_directory.json` for multiple `agent:main:slack:dm:<D...>:<thread_ts>` entries. Hermes Slack defaults `dm_top_level_threads_as_sessions` to `true`, so each top-level DM message can become its own session/thread (`slack.py::_dm_top_level_threads_as_sessions`). To make one DM carry one continuous Hermes context, configure `platforms.slack.extra.dm_top_level_threads_as_sessions: false` and restart the gateway. Caveat: Slack Assistant’s UI may still present items under “내역”; this config primarily changes Hermes session continuity and reply routing, not Slack’s native Assistant UI design.
