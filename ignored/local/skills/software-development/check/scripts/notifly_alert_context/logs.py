...
def collect_surrounding_log_contexts(
    ...
    for row in sample_rows:
        ...
        try:
            ...
            # payment-executor logs contain "Payment details" arrays with project_id ~15+s before the ERROR line
            before = 25 if payment_mode else 5
            after = 15 if payment_mode else 5
            stream_context = fetch_log_stream_context_events(session, group, stream, trigger_ms, before_seconds=before, after_seconds=after)
...
""".strip()
    ...
    recent_trigger_contexts = collect_surrounding_log_contexts(session, groups, recent_rows, payment_mode=payment_mode)
...
""".strip()
            ...
            current_trigger_contexts = collect_surrounding_log_contexts(session, groups, current_rows, payment_mode=payment_mode)
