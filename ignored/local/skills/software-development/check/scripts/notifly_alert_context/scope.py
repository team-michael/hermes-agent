...
def build_scope_attribution(
    ...
    if pair_labels:
        ...
        campaign_label = ''
    payment_product_names = detected.get('payment_product_names') or []
    journey_values = unique([*user_journey_ids, *user_journey_refs])

    scope_label_parts: List[str] = []
    if campaign_label:
        scope_label_parts.append(f'캠페인 {campaign_label}')
        scope_kind = 'campaign'
    elif journey_values:
        journey_part = f'유저여정 {", ".join(journey_values[:5])}'
        if len(journey_values) > 5:
            journey_part += f' 외 {len(journey_values) - 5}개'
        scope_label_parts.append(journey_part)
        scope_kind = 'user_journey'
    if payment_product_names:
        payment_part = f'결제 상품: {", ".join(payment_product_names[:5])}'
        if len(payment_product_names) > 5:
            payment_part += f' 외 {len(payment_product_names) - 5}개'
        scope_label_parts.append(payment_part)
        if not campaign_label and not journey_values:
            scope_kind = 'payment'
    if not scope_label_parts:
        scope_kind = 'unknown'
...
def merge_scope_detections(
    ...
    # Collect payment product names from both raw log text and current error details
    payment_product_names: List[str] = []
    raw_all_text = '\n'.join([
        text,
        current_detail_text,
        '\n'.join(current_context_text_parts),
    ])
    payment_product_names = detect_payment_product_names(raw_all_text)
    ...
    return {
        ...
        'payment_product_names': payment_product_names,
    }
