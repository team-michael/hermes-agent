...
**Pitfall — Sentry email alert pipeline matching its own payload**: The `ops-email-receiver` Lambda writes parsed Sentry email alerts to a dedicated log group (`/aws/ecs/notifly-services-prod/web-console/sentry`). A broad `%ERROR%` metric filter on that log group matches the S-issue `title` (`"Error"`, `"SyntaxError"`) and `"level":"error"` inside the JSON payload, causing the alarm to fire whenever any Sentry alert arrives. The Lambda itself is healthy; the real errors are `web-console` Next.js issues tracked in Sentry. When the helper returns empty `current_trigger_contexts`, fall back to `aws logs filter-log-events` bounded by the exact metric-datapoint window because Logs Insights lags metric-filter ingestion. See `references/sentry-email-alert-pipeline-false-positives.md` for the payload-analysis workflow (page/feature/error extraction, project mapping, and aggregation) plus scope extraction via `productId` and DynamoDB GSI lookup.
...
**Pitfall — `cafe24-token-refresher` deleted/completed mall recurring `invalid_client` ERROR**: The `cafe24_integration` DynamoDB table is scanned by `getCandidatesToRefreshAccessToken` with a filter on `token.expires_at` but **no `status` filter**. Malls with `status: deleted` or `status: completed` remain in the table; when their Cafe24 app is uninstalled, every scheduled refresh attempt produces `ERROR Failed to get fresh token <mall_id> invalid_client Uninstalled app.`. Lambda runtime `Errors=0` because the failure is handled and the invocation continues with other candidates. This is a handled external-provider rejection, not a service bug. Classification: `no_action` when isolated; `needs_fix` if the same mall recurs daily because the structural noise costs operational attention. Immediate remediation: delete the row from `cafe24_integration`. Structural remediation: add `FilterExpression` on `status` in `services/lambda/cafe24-token-refresher/lib/ddb.js` to exclude non-active records. See `references/lambda-consoleerrors-handled-business-rejection.md` § "cafe24-token-refresher: deleted/completed mall token refresh failure" for scope extraction, bounded log check, and exact code targets.
