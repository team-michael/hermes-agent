...
- For log-derived alarms, the strongest evidence is the exact breaching datapoint window from CloudWatch `stateReasonData` (`logs.current_alarm_window.basis == latest_alarm_state_reason_data`), not a broad recent/30-minute scan, the alarm name, or frequency. If `logs.current_error_details` exists, the final Korean answer must describe the concrete triggering log from that exact window before discussing 7d/30d frequency or threshold sensitivity. Treat `logs.recent_trigger_contexts`, `logs.top_signatures`, and 7d/30d samples as baseline only; never use them as current root cause or scope evidence.
...
- **DB alert SQL fingerprint requirement**: The `원인:` line for any DB-shaped alert must include the exact SQL fingerprint (e.g., `SELECT users_<hash>.notifly_user_id ... LIKE $1 LIMIT $2`) discovered from Performance Insights, `pg_stat_statements`, or DB logs. Table suffixes (`users_<hash>`, `event_intermediate_counts_<hash>`, etc.) must be mapped via DynamoDB `project` and the resulting `product_id`/`name` must appear in `원인:` or `범위:`.
- **DB alert action item requirement**: The `액션 아이템:` for DB-shaped alerts must name a concrete SQL-level target: index to check (`EXPLAIN ANALYZE`), parameter binding pattern, or table partition/scan hypothesis, rather than generic "optimize DB" advice.
...
- Use exactly these Korean labels, in this order:
  - `원인:` alarm-triggering system-level cause plus code-level cause. Include both in one compact line; if one is unknown, say why briefly. **For DB-shaped alerts**, append the exact SQL fingerprint (truncated to ≤180 chars) from Performance Insights, `pg_stat_statements`, or DB logs, and the mapped product/project name derived from any sharded table suffix.
...
5. For log lines containing both `Project Id` and `Campaign Id`, treat the pair as the primary campaign scope only when it appears in `logs.current_error_details` or the exact breaching datapoint's trigger line. Current breaching-datapoint pairs outrank 7d/30d historical signatures. Do not promote IDs from `logs.recent_trigger_contexts`, `logs.top_signatures`, broad recent samples, or neighboring `surrounding_lines` unless the trigger itself is a generic error marker such as `ERROR`/`severity: ERROR` and the neighboring line is the same error block.
...
8. For web-console service logs where the triggering API endpoint (e.g., `/api/liquid/template`) does not log `project_id` directly, inspect surrounding access log lines for the `Referer` header path (`/console/products/<productId>/...`). Map `<productId>` via DynamoDB `project` table GSI `product_id-project_id-index`. The same `product_id` may map to multiple `project.id` values. See `references/web-console-scope-attribution-via-access-logs.md`.
9. For api-service structured `error-response` logs where `projectId` is explicitly `"unknown"` (e.g., `POST /authenticate` 400 `"Missing required fields"`), do not search for alternative scope attribution. The authentication/validation middleware logs `"unknown"` because project resolution happens after the early validation that rejects the request. This is expected and correctly untraceable to a project.
10. For service-wide, Lambda/ECS, RDS, SQS, Redis, or broad metric-filter alerts with no per-project evidence, state in Korean that the project and campaign/user journey are unknown, and add a Korean service-wide or infra-wide marker when that is the correct scope.
   - For shared-pipeline SQS queues (e.g., `kinesis-record-dispatcher-queue`) where messages aggregate records from all projects, the Lambda consumer logs typically contain only `START/END/REPORT` lines with no per-project IDs. Do not force a project scope; explicitly say the scope is infra-wide common pipeline delay. For the producer/consumer pattern, recurrence details, and Lambda throughput bottleneck analysis, see `references/kinesis-record-dispatcher-queue-spike.md` and `references/kinesis-record-dispatcher-queue-throughput.md`.
...
**Pitfall — alarm name ending in `... console error` not parsed by text detector**: alarm names consisting of `<log-group-or-service> console error` (e.g., `/aws/ecs/notifly-services-prod/segment-publisher console error`) may return `detected.alarm_name: null` because the helper text parser has no heuristic for the trailing `console error` suffix. The parser may extract the log group path but fail to construct the full alarm name. Pass `--alarm-name` explicitly.
...
**Pitfall — Fargate log stream expiration before investigation (segment-publisher):** When following the `segment-publisher slow eic query` fallback for empty trigger contexts, the segment-publisher Fargate task may have already stopped by the time the investigation runs. Its log stream then disappears from `describe_log_streams` results and `filter_log_events`/`Logs Insights` queries return zero even though the metric filter demonstrably breached. If the companion alarm `segment-publisher long running alam` (namespace `Custom/segment-publisher`) is in ALARM with a datapoint in the same time window, treat this as strong evidence for Pattern B (`[WARN] Processing took longer than expected`) without requiring the exact trigger log line. See `references/segment-publisher-slow-eic-query-noise.md` § "Companion-alarm shortcut rule".
...
**Pitfall — `can_answer_root_cause: false` with actionable `current_trigger_contexts`**: The helper may return `can_answer_root_cause: false` because `current_error_details` is empty even when `current_trigger_contexts` already contains a clear, specific error signature. When `current_trigger_contexts` shows a recognizable signature (e.g., `FailedToUploadImageException`, `InvalidImageFormatException`, `The maximum number of registered templates.`, `080 unsubscribe number missing`, `AbortError: message is aborted` / `abort_message()`, `[Symbol(errored)]`, `de_CommandError` from `@aws-sdk/client-ses`), check whether it matches a known reference pattern first. If it does, the root cause is already known — the missing `current_error_details` is a structural gap, not a blocking unknown. In this case, bypass the generic `required_followups`, use the reference to classify the alert, and produce the final answer directly. Only run manual follow-up if the trigger context is truly ambiguous (generic `severity=ERROR` with no code path or provider name) or if scope attribution is still required for mandatory fields.

**Pitfall — `can_answer_root_cause: false` with empty `current_trigger_contexts` on a documented periodic alarm**: When `current_trigger_contexts` is empty but the alarm history shows a perfectly stable daily recurrence (e.g., 30+ consecutive days with transitions at nearly the same clock time) and the alarm family is already covered by a `references/` entry, do not run broad manual log searches as a mandatory follow-up. Instead: (1) verify any companion alarm in a dedicated namespace that covers the same signal (e.g., `segment-publisher long running alam` for batch-processing WARNs), (2) confirm the current day's count and timing match the documented baseline, and (3) classify directly from the reference pattern. Run bounded manual traces only when the daily count, timing, or companion-alarm behavior has materially deviated from the baseline, or when the scope cannot be recovered from prior-day evidence and the mandatory `범위` field is incomplete.
...
5. Performance Insights `db.load.avg` grouped by `db.sql` on the offending instance
   ...
   - **pg_stat_statements fallback**: When PI is unavailable or insufficient, run read-only `pg_stat_statements` on the writer for top `mean_exec_time`/`max_exec_time` queries filtered by Notifly table families (`event_intermediate_counts_*`, `users_*`, `delivery_result_*`). Include the exact query text (truncated to 180 chars) with `calls`, `mean_exec_time`, `max_exec_time`, and `rows` in the final `원인:` line. Map every sharded table suffix to a project/product via DynamoDB before filling `범위:`.
...
**Pitfall — access-log benign substring matching coarse filter**: a metric filter such as `%ERROR|Exception%` may match benign substrings embedded in HTTP access logs, e.g. a query parameter `templateName=service_error` or `templateName=receipt-error-AOSup-0519`, or a path segment containing `error`. The resulting log line is a normal 200/304 request, not an application error. When `logs.current_error_details` is empty and the trigger context shows only access logs, run a follow-up Logs Insights query that filters out known benign patterns (e.g., `service_error`, `receipt-error`, `error.html`) to confirm whether any real ERROR logs exist in the alarm window. See `references/ecs-console-error-false-positive-patterns.md`.

**Pitfall — Sentry email alert pipeline is an intentional proxy, not a false positive**: The `ops-email-receiver` Lambda receives Sentry alert emails via SES and writes parsed payloads to the dedicated log group `/aws/ecs/notifly-services-prod/web-console/sentry`. A broad `%ERROR%` metric filter matches literal strings inside the JSON payload (`"title":"Error"`, `"level":"error"`, etc.), so the alarm fires on every Sentry alert arrival. This is expected behavior — the Lambda intentionally logs at ERROR level to make alerts visible. Do **not** classify this alarm as a pipeline false positive or Lambda health issue. The investigation target is the **actual Sentry issue content** embedded in the proxied JSON: parse `sentryAlert.issue.title`, `message`, `transaction`, `request.url`, and `tags.handled`, then report which web-console page/feature is failing and which Notifly project is affected. Extract the Notifly `productId` from `request.url` (e.g., `/console/products/<productId>/...`) and map it via DynamoDB `project` table GSI `product_id-project_id-index` (prefer `dev=false`). Do **not** use the Sentry `project.id` (e.g., `4506086856196096`) as a Notifly `project_id`; it is the Sentry organization project identifier. When the helper returns empty `current_trigger_contexts`, fall back to `aws logs filter-log-events` bounded by the exact metric-datapoint window because Logs Insights lags metric-filter ingestion. See `references/sentry-email-alert-pipeline-false-positives.md` for the full payload-analysis workflow. Default status is `no_action` because the pipeline itself is healthy and individual Sentry issues are tracked in the Sentry (`greybox`) organization, unless a specific error title/transaction pair is sharply increasing across multiple production projects.
...
**Pitfall — web-console scope attribution from access log Referer**: When a `web-console` console error trigger lacks explicit `project_id` (e.g., `/api/liquid/template` endpoint), check surrounding access log lines for the `Referer` header path (`/console/products/<productId>/...`). Extract the `<productId>` slug and map it via DynamoDB `project` table GSI `product_id-project_id-index`. The same `product_id` may map to multiple `project.id` values. See `references/web-console-scope-attribution-via-access-logs.md`.

**Pitfall — `shortenNotiflyUserId` BigInt parse failure on non-hex test IDs**: When `current_trigger_contexts` shows `[ERROR] SyntaxError: Cannot convert 0x<invalid> to a BigInt` with IDs like `0xtest_80`, `0xsdfsdf`, or hyphenated UUID fragments (`0x52d1f43f-c4f`), the root cause is `shortenNotiflyUserId()` → `encode()` → `BigInt('0x' + partialUuid)` failing because the user ID is not valid hexadecimal. The `shortenNotiflyUserId` function catches the exception and returns the original ID unchanged (`packages/util/src/util/shortenNotiflyUserId.ts:10-15`), but logs `console.error(e)` which trips the `%ERROR%` metric filter. Message rendering and delivery continue normally. Scope is usually internal test projects (e.g., `michael`). Classify as `no_action` when only test IDs are present. See `references/shorten-notifly-user-id-bigint-false-positive.md` for the exact code path, classification rules, and remediation options.
...
6. **Lambda throughput bottleneck analysis** (for `ApproximateAgeOfOldestMessage` alarms):
   ...
   - for the `kinesis-record-dispatcher-queue` specific burst pattern driven by `segment-publisher` scheduled campaigns, see `references/kinesis-record-dispatcher-queue-throughput.md`
...
See `references/sqs-dlq-new-consumer-deployment-failure.md` for bounded trace commands.
- **DLQ creation alarm + healthy Lambda + maxReceiveCount=1**: When a DLQ creation alarm (`<queue>-dlq has been created`) fires, the consumer Lambda is healthy, and `maxReceiveCount=1`, the DLQ entries are from zero-retry structural policy, not code bugs. See `references/sqs-dlq-creation-alarm-lambda-healthy.md` for bounded trace commands and classification.
- **`kinesis-record-dispatcher-queue` DLQ pattern**: A specific case of the structural `maxReceiveCount=1` pattern. DLQ messages are `kds-consumer` triggering events on `notifly-triggering-events-stream` and carry `project_id`/`campaign_id`/`experiment_id`/`variant_id` directly in the JSON payload. See `references/sqs-dlq-kinesis-record-dispatcher-pattern.md` for extraction commands and scope mapping.
- **Pitfall — helper returns `lambda: null` for SQS queue alarms**: The helper does not auto-resolve the Lambda consumer from an SQS queue name alone (e.g., `kinesis-record-dispatcher-queue too high approximate age`). When `lambda` is null, manually run `aws lambda list-event-source-mappings` filtered by the queue ARN to find the consumer, then check `BatchSize`, `MaximumConcurrency`, `Errors`, `Throttles`, and `Duration` before concluding root cause. Without this step, throughput bottleneck alarms may be misclassified as unknown scope or unverified code failure.

See `references/rds-aurora-replica-recovery-conflict.md` for bounded trace commands, scope extraction from `users_<project_id>` table references, and classification guidance.
...
**Pitfall — INFO log volume drowning out ERROR trigger in helper signatures**: Some Lambdas (e.g., `kakao-brand-message-delivery`) emit large INFO request/response bodies for every invocation while producing only 1-2 ERROR lines per alarm window. The helper's `current_top_signatures` and `current_trigger_contexts` may then contain only INFO patterns and report `current_error_details` as missing. When the metric filter demonstrably breached but the helper shows no ERROR detail, run a bounded `filter_log_events` with `filterPattern='ERROR'` on the exact alarm datapoint window to isolate the trigger line. Do not conclude the alarm is unclassifiable just because the helper's top signatures are INFO-only.
...
**Pitfall — DLQ alarms with unknown scope require message-body inspection**: For SQS DLQ alarms, the helper has no CloudWatch log group to query and does not read DLQ message bodies. When `scope_attribution.scope_kind` is `"unknown"` and the alarm is DLQ-shaped, perform a bounded `aws sqs receive-message` (read-only, short visibility timeout) on the DLQ to extract `project_id`/`campaign_id` from the JSON payload. Map `project_id` via DynamoDB `project` before declaring infra-wide scope. This is especially important for `kinesis-record-dispatcher-queue-dlq` where messages carry `.records[].data.project_id` and `.records[].data.campaign_id` directly. See `references/sqs-dlq-kinesis-record-dispatcher-pattern.md`.

**Pitfall — Node.js deprecation warnings can trigger `%ERROR%` metric filters**: Node.js 22.x cold-start deprecation warnings (e.g., `[DEP0040] DeprecationWarning: The punycode module is deprecated...`) may contain the literal string `ERROR` in their output line, breaching coarse `%ERROR|Status: timeout%` filters despite being benign warnings with no runtime failure. The first matching line in a scheduled Lambda invocation often has `undefined` as the request ID. Cross-check `AWS/Lambda Errors`; if zero, this is a false positive. In scheduled Lambdas like payment-executor, the deprecation warning often appears alongside handled business rejections caught in `try...catch` (e.g., `PaymentError: Payple payment failed`). See `references/nodejs-deprecation-warning-lambda-consoleerrors-false-positive.md` for triage and ranked remediation steps (including the `NODE_NO_WARNINGS=1` fix and the Terraform `ignore_changes` constraint).
...
**Output side**: `filter-log-events` also returns `.events[].timestamp` in **milliseconds**. `jq`'s `todate` expects seconds, so `(.timestamp | todate)` produces wildly wrong dates. Divide by 1000 first: `(.timestamp / 1000 | todate)`. See `references/cw-logs-timestamp-jq-pitfall.md`.
...
**Pitfall — `kds-consumer` notif_auth_status_enum validation failure (device upsert)**: When `current_trigger_contexts` shows `invalid input value for enum notif_auth_status_enum: "authorized"` on `INSERT INTO device_<project_id>`, the root cause is an unvalidated `event_params.notif_auth_status` arriving from the client. Notifly expects numeric string enum values (`'-1','0','1','2','3'`), but direct HTTP API callers or custom SDK integrations sometimes send platform-native names like `"authorized"`. The Lambda catches the error and continues (`AWS/Lambda Errors=0`). Scope is extractable from the table suffix (`device_<project_id>`). Classify as `no_action` when sporadic and isolated to one project; `needs_fix` if it spikes across multiple projects after a deployment. See `references/kds-consumer-notif-auth-status-enum-failure.md` for exact SDK mappings, PG enum values, and the code path in `device_utils.ts`/`event_utils.ts`.
...
See `references/sqs-dlq-new-consumer-deployment-failure.md`.
