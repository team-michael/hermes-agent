...
**HARD RULE before anything else in this file**: never run `get-log-events`/`get_log_events`
on `segment-publisher` (or any service that logs the full inbound SQS payload) without
first checking whether a targeted `filter-log-events --filter-pattern` can answer the
question instead. `get-log-events` returns every line in the stream verbatim, and for
segment-publisher the first lines of every batch are `Received event: {...}` — a JSON
blob containing base64-encoded FCM service-account private keys, Kakao `sender_key`/
`auth_token`, and VAPID private keys. Actually observed: pulling `get-log-events --limit 50`
across 2-3 candidate streams dumped ~500 lines of raw credential payload into tool output
in a single call. That is a live secrets-exposure bug, not a hypothetical — treat it as
severity-1 for your own output discipline. Concretely:
- Prefer `filter-log-events --filter-pattern '"exact phrase"'` (e.g. the WARN/ERROR trigger
  text) so CloudWatch does the filtering server-side and only matching lines return.
- If you must fall back to `get-log-events` (e.g. Logs Insights ingestion lag, see below),
  cap `--limit` to single digits, and only reference/quote the specific
  `campaignId`/`project_id`/error-signature fields you extract — never paste or forward the
  `Received event:` line itself.
- If a `Received event:` line does end up in a tool result, do not copy any part of it
  (even truncated) into the final Slack/user-facing answer.
...
- **`Received event:` lines in segment-publisher logs contain full provider credentials:** The segment-publisher service logs the entire SQS event payload as `Received event: {...}` at the start of each batch. This JSON includes FCM service account private keys (base64-encoded), Kakao sender keys, VAPID keys, and other secrets. When using `get-log-events` to find project/campaign context, the `Received event:` line will be dumped into terminal output. Prefer `filter-log-events` with targeted patterns (`project_id`, `campaign_id`, `Processing took longer`, `Used user property names`) to extract only relevant context lines. If `get-log-events` must be used, do not paste or process the `Received event:` line content — extract only the `project_id` and `campaign_id` fields needed for scope attribution and discard the rest.
