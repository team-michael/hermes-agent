...
7. **`[expected] InvalidImageShapeException(가로:세로 비율은 2:1여야 합니다 , 가로는 최소 500px, 세로는 최소 250px 이상이어야 합니다, <filename>)`** — uploaded campaign/brand-message image fails the required aspect ratio (2:1) or minimum dimensions (500x250px). Distinct from pattern 2 (`InvalidImageFormatException`, unsupported format) — this one is a shape/dimension check. Note the literal `[expected]` prefix baked into the log line itself.

## General heuristic: `[expected]` log prefix

When a current-alarm-window trigger line is explicitly prefixed with `[expected]` (or similar code-level markers like `handled`, `tags.handled: "yes"`), treat this as a strong, generalizable signal that the log is an intentional handled-rejection log, not a service fault — even if the specific exception class is not yet catalogued above. Do not wait to find the exact string in this reference before classifying; the prefix itself, combined with single/low-count occurrence and fast alarm recovery (OK within ~1 minute), is sufficient evidence for `no_action`. Still verify: (1) no other ERROR patterns coexist in the same window, (2) Lambda/ECS runtime error metrics (if any) are 0, (3) the alarm actually recovered.
