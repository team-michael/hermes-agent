...
**Also confirmed on `user-journey-kinesis-record-dispatcher`** (2026-07-08 session, alarm `user-journey-kinesis-record-dispatcher lambda error`, threshold breach 5.0 at 00:15 UTC, self-recovered by 00:17 UTC). This Lambda's `index.ts` calls the *same* shared `putRecords()` from `@notifly/kinesis` (`packages/kinesis/src/lib/kinesis.ts:160-166`) targeting `StreamName.USER_JOURNEY_NODE`, so it inherits the identical partial-failure-swallowed-silently anti-pattern. Any Lambda in `services/lambda/*` that imports `putRecords` from `@notifly/kinesis` is subject to this same failure mode — check the call site's `StreamName` argument to identify which stream/dispatcher is affected before assuming it's specific to the base `kinesis-record-dispatcher` function. Helper text-parser note: for this variant, `--text` alone (no `--alarm-name` needed) correctly auto-detected `detected.alarm_name`, unlike the base function's alarm name below — don't assume every `*-kinesis-record-dispatcher lambda error` alarm needs an explicit `--alarm-name` override; try without it first.
