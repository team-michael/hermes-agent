...
#### Pattern C: duplicate `X-Request-Id` rejection (Kakao API 400 `[4131]`)

- Trigger string: `Kakao Bizmessage direct send failed: <projectId>/<campaignId>/<recipientHash> KakaoBizmessageApiError: Kakao API error: 400 [4131] 중복 requestId (X-Request-Id)`
- Code path: `services/lambda/kakao-brand-message-delivery/lib/kakao_bizmessage/api_client.ts` `sendKakaoApiRequest()`. Status 400 is not in `isRetryableStatus()` (`retry_policy.ts`), so the `KakaoBizmessageApiError` is thrown on the first attempt with no retry. The caller catches it, `console.error`s the message, classifies it via `error_classifier.ts`, and inserts a row into `delivery_failure_log_<project_id>` (confirmed by the adjacent `SendFailureLogInsertQuery` INFO line in the same millisecond).
- Why it happens: Kakao's Bizmessage API rejects a request whose `X-Request-Id` header matches one it has already seen within its idempotency window. `request_id_generator.ts` builds the ID from `KST-date-epochMs-sequence-random`, which is not expected to collide under normal single-attempt sends; a 400 `[4131]` on the very first attempt (not a retry) points at either a concurrent duplicate invocation for the same recipient/campaign or a widened idempotency window on Kakao's side, not at a bug in the generator itself.
- Invocation health: `AWS/Lambda Errors == 0`, `Throttles == 0`. The Lambda completes normally; only the single recipient's send fails and is recorded as a delivery failure.
- Scope extraction: the trigger line carries `projectId/campaignId/recipientHash` directly — map `projectId` via DynamoDB `project`.
- Classification: `no_action` when isolated (single recipient, Lambda healthy, failure correctly logged). Escalate to `needs_fix` only if the same campaign shows repeated `[4131]` rejections across many recipients (would suggest a real duplicate-send bug rather than one-off idempotency overlap).
- **Log-search pitfall**: when isolating this signature, do not filter `filter_log_events`/Logs Insights by the bare error code as a generic numeric substring (e.g. `"4131"`) — it also matches unrelated digits inside EMF metric JSON (`_aws.Timestamp` epoch values), producing false positives. Filtering by the Korean phrase (`"중복 requestId"`) can also return zero matches even when the line exists (encoding/escaping issues through boto3 `filterPattern`). The reliable term is the distinctive English exception class name, e.g. `filterPattern='"KakaoBizmessageApiError"'` — it is unique to this failure family and not present in unrelated INFO/EMF lines.

#### Pattern D: NHN Cloud Kakao Bizmessage field validation rejection (`resultCode: -3002`, "Field is not valid.")

- Trigger string: `ERROR Error: { "header": { "resultCode": -3002, "resultMessage": "Field is not valid.", "isSuccessful": false } } at request (/var/task/lib/nhncloud/send_kakao_brand_message.js:64:19)`
- Code path: `services/lambda/kakao-brand-message-delivery/lib/nhncloud/send_kakao_brand_message.ts:63-92`, function `request()`. When the NHN Cloud API response has `header.isSuccessful === false`, the code throws `new Error(JSON.stringify(response))`. The `catch` block checks `SUPPRESSIBLE_ERROR_MESSAGES`; if the message is not in that suppress list it falls through to `console.error(err)` (line 89) before re-throwing. This is the same request-level guard used for the 080-unsubscribe and sender-info patterns above, just a different NHN Cloud rejection code.
- Why it happens: `resultCode: -3002` / `"Field is not valid."` is NHN Cloud's own request-field validation rejection (malformed `sender_key`, `template_code`, recipient field, or similar in the `send/basic/batch` request body), not a Notifly bug. This is an external-provider-side rejection, analogous in class to the "maximum number of registered templates" and Kakao image-upload validation errors documented elsewhere in this skill.
- Invocation health: confirm `AWS/Lambda Errors == 0` and `Throttles == 0` for the alarm window — if so, the Lambda completed normally and only that batch's send failed (recorded via `prepareSendFailureLogToInsert` / `delivery_failure_log_<project_id>`, not silently dropped).
- **Scope-attribution limit (per-line)**: the bare ERROR line itself carries no `projectId`/`campaignId`. The adjacent INFO log `"Kakao Bizmessage request body"` carries `sender_key` and `template_code`, but the `project` DynamoDB table has no GSI on `kakaoSenderInfo.senderKey`, and a full table `Scan` filtering nested map attributes is not a reliable/bounded lookup (do not attempt a blind full-table scan as a scope-recovery step).
- **Scope recovery technique that works**: do not stop at the bare ERROR line. Pull the `filter_log_events` window slightly wider (a few seconds before the ERROR) on the **same log stream**. Each Lambda invocation logs `INFO Received event: {...}` and `INFO Received event from SQS: { "project_id": "...", "resource_type": "campaign"|"user_journey", "campaign_id": "...", ... }` at the start of the invocation, before the NHN Cloud request/response. Match the ERROR's `logStreamName` back to its invocation's SQS payload line to recover `project_id`/`campaign_id`/`resource_type` directly — no DynamoDB scan needed. Then map `project_id` via DynamoDB `project` (`GetItem` with projection on `id, product_id, name`).
  ```bash
  python3 -c "
  import boto3, datetime
  session = boto3.Session(region_name='ap-northeast-2')
  ...
  resp = logs.filter_log_events(
      logGroupName='/aws/lambda/kakao-brand-message-delivery',
      startTime=start, endTime=end,
      filterPattern='\"project_id\"',
      limit=30,
  )
  for e in resp.get('events', []):
      if 'Received event from SQS' in e['message']:
          print(e['message'][:400]); print('---')
  "
  ```
  **Multi-project pitfall**: a single 60-second alarm window commonly contains a *batch* of invocations spanning several distinct `project_id` values (observed: two projects mixed in one window, one dominant by invocation count). Do not assume single-project scope from one sample line — pull several matching SQS payload lines across the window and report the dominant project plus note if others are mixed in.
- **Frequency — do not undercount from alarm transitions alone**: alarm `OK→ALARM` transition counts (e.g. 30d 6 / 7d 3 / 1d 2 / 10m 1) look sparse because `datapoints_to_alarm=1`/`period=60` with `treat_missing_data=missing` only transitions when a 60s bucket crosses the sum≥1 threshold — it does not reflect underlying volume. Cross-check `logs.count_7d`/`count_30d`/`daily_counts_30d` from the Logs Insights `%ERROR|Status: timeout%` filter: this pattern has been observed producing **24k-142k matching ERROR log lines per day** sustained over 30 days, with 25-30 raw `resultCode: -3002` ERROR lines inside a single breaching 60s window. This is a chronic high-volume rejection stream, not an isolated/sporadic event — say so explicitly in the `빈도` field (alarm-transition count for the trip, but log-volume count for the underlying rate) rather than implying rarity from the transition count alone.
- Classification: `no_action` when Lambda `Errors == 0` even at this high per-line log volume, because each rejection is a handled per-recipient send failure recorded to `delivery_failure_log_<project_id>` and the batch continues; the alarm is tripping on ERROR log *volume*, not on any functional failure. Escalate to `needs_fix` only if the daily log-volume trend is worsening materially beyond this established high baseline, or if `AWS/Lambda Errors > 0` appears alongside it.
- Action target (if noise threshold crossed): downgrade this specific NHN Cloud rejection to `console.warn` by adding the resultCode/message pattern to `SUPPRESSIBLE_ERROR_MESSAGES` in `send_kakao_brand_message.ts`, consistent with how other expected NHN Cloud rejections are already suppressed in that same file. Given the sustained tens-of-thousands-per-day volume, also consider narrowing the `kakao-brand-message-delivery lambda error` metric filter/alarm period so it is not tripped by ordinary per-recipient rejection bursts (see Terraform `infra/terraform/prod/ap-northeast-2/lambda/functions.tf` alarm block, `period=60`/`threshold=1.0`).

- **Escalation check — single-campaign 100%-failure variant vs. baseline noise**: the "chronic high-volume" framing above assumes the `-3002` rejections are spread thinly across many campaigns/recipients (per-recipient noise). Before defaulting to `no_action`, isolate whether **one specific recurring campaign** is failing on *every* invocation of *every* recipient batch, every day. This is a materially different, actionable signal: it means that campaign's template/sender fields have been broken since some point and zero customers on it are receiving messages, not that the Lambda has generic rejection noise.
  - Verification technique: grep the alarm-window log stream for the specific `campaign_id` (pulled from the adjacent `Received event from SQS` INFO line per the scope-recovery technique above), e.g. `filterPattern='"Xs43jM"'`, and confirm every batch for that campaign_id in the window hit the same `-3002` error with no successful sends interleaved.
  - Daily-volume trend technique: pull the alarm's own `ConsoleErrors` metric with `Period=86400, Statistics=['Sum']` over ~30 days. Alarm `OK→ALARM` transition counts under-report because `datapoints_to_alarm=1`/`period=60` only trips once a 60s bucket crosses threshold — a day with 288 raw ERROR lines and a day with 2 both count as "1 alarm." The daily Sum series is what reveals an escalating trend (e.g. observed 2 → 2 → 288 → 46 across four trigger days over 3 weeks) versus flat low-volume baseline.
  - Classification: escalate to `needs_fix` when a single `project_id`/`campaign_id` pair shows sustained or growing same-day-every-day full-batch failure — this is real, ongoing customer-facing non-delivery for that campaign, not alarm noise. Action target is the campaign's own send fields (sender_key/template_code/image/link for that specific `template_code`), not just a log-level downgrade: point the follow-up at the web-console campaign editor for that `project_id`/`campaign_id` and at NHN Cloud's template field spec, in addition to the generic `SUPPRESSIBLE_ERROR_MESSAGES` noise-reduction option.

### payment-executor: handled Payple `PaymentError` rejections (wallet/pay-as-you-go batch)

- Trigger strings: `Failed to execute payment for <label>: PaymentError: Payple payment failed: [SPCD0002] 결제가능 최저금액은 100원 입니다.`, `Failed to execute pay-as-you-go payment for <label>: PaymentError: Payple payment failed: [SPCD0002] ...`, `Failed to execute payment for <label>: PaymentError: Insufficient credits`
- Alarm: `payment-executor lambda error`, namespace `ConsoleErrors`, metric filter `%ERROR|Status: timeout%` on `/aws/lambda/payment-executor`.
- Why it happens: `payment-executor` runs a **daily scheduled batch** (fires ~00:00-00:07 UTC / 09:00-09:07 KST every day) that retries wallet and pay-as-you-go payments via the Payple gateway. When a wallet balance is below Payple's minimum chargeable amount (100 KRW, code `SPCD0002`) or the account has insufficient credits, the code throws a `PaymentError`, logs it via `console.error` in a `try/catch`, and continues to the next candidate. `AWS/Lambda Errors` and `Throttles` are 0 across the full 7-day window — the invocation always completes normally.
- Scope: these ERROR lines carry a wallet/customer label but no Notifly `project_id`/`campaign_id`/`user_journey_id`. Treat as service-wide daily batch noise, not project-scoped, unless a future occurrence shows an ID.
- Frequency baseline observed (2026-07-11): 30d 9 alarm transitions / 7d 3 / 1d 1 / 10m 1, `daily_alarm_counts` roughly 1-22/day, no growth trend. Recurs at nearly the same clock time daily — treat as a time-of-day scheduled-batch signature per the SKILL.md "Time-of-day recurrence" rule, not random traffic.
- Classification: `no_action` for isolated/baseline occurrences (Lambda healthy, rejections are legitimate business outcomes). Escalate to `needs_fix` only if daily volume trend clearly worsens beyond this baseline or a non-`PaymentError` signature (e.g. an actual `AWS/Lambda Errors > 0`) appears in the same window.
- Action target if noise crosses a tracked threshold: downgrade the caught `PaymentError` branch from `console.error` to `console.warn` in the payment-executor Lambda's payment-retry loop (exact file/line not yet located — next lookup target is `services/lambda/payment-executor/` for the `catch` block around the Payple call).
