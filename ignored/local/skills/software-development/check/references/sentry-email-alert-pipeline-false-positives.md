...
- `NoSuchKey` (`The specified key does not exist.`) at `POST /api/s3/get_segment_data` — Server-side `readSegmentCSVFileFromS3` (`services/server/web-console/src/pages/api/lib/s3.ts`) calls `GetObjectCommand` on bucket `csv-segment` with key `${projectId}/${filename}` while loading a CSV preview for a saved user segment; the object is missing (expired/deleted upload, or stale filename reference). Sentry issue `7637055917` (New issue, `handled: yes`, 2026-07-28). **Scope recovery pitfall**: `projectId`/`filename` are sent as a JSON POST body (`fetch('/api/s3/get_segment_data', {method:'POST', body: JSON.stringify({projectId, filename})})` from `csv_utils.ts`), not as a URL path/query segment, so `request.url`/`tags.url` in the Sentry payload is just the bare route (`https://console.notifly.tech/api/s3/get_segment_data`) with no identifiable project — scope is service-wide (서비스 공통, web-console 세그먼트 CSV 다운로드 기능), campaign/user-journey 특정 불가. Caught exception (`handled: yes`), no server crash, single failed CSV read, no data loss. Isolated first occurrence, not in 7d/30d top signatures → classify `no_action`. Escalate to `needs_fix` only if this title+transaction recurs (would suggest S3 lifecycle deleting segment CSVs prematurely or a stale-filename bug in the segment builder UI, not a one-off).
