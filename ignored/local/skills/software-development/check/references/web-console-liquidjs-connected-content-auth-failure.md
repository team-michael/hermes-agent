...
## Recurrence log — same Supabase project, Liquid Playground variant

- **2026-07-02 06:31 KST~ (UTC)**: alarm fired twice within ~1 minute (`user_id=324288`, then `user_id=13729`), both from `POST /api/liquid/template` via Referer `https://console.notifly.tech/ko/console/products/qmarket/utils/liquid-playground`. Same Supabase URL `https://htcxkbijmiptoubmkhkm.supabase.co/functions/v1/personalization`. Project resolved via DynamoDB `product_id-project_id-index` on `qmarket` → prod `project.id = f2e198e2448959908fe4f8e540f4057f` (dev=false item; a `dev=true` shadow project with the same product_id also exists — prefer `dev=false` for the customer-facing scope).
- Confirms this Supabase project's personalization token issue is a recurring, unresolved customer-side credential problem (first seen 2026-06-17, spiked 2026-07-01, recurring 2026-07-02), not a one-off. Still `no_action` per-incident since the console route degrades gracefully (500 on the playground call only, no campaign/user-journey delivery impact), but worth flagging to the `qmarket` project owner directly if it keeps recurring — the Supabase key needs to be rotated/fixed on their end.
