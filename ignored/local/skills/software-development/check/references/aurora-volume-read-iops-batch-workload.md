...
## Companion-alarm shortcut (RETIRED ~2026-06 — do not rely on this alone)

**Update (2026-07-04 session)**: `describe-alarms` with `contains(AlarmName, 'segment-publisher') && contains(AlarmName, 'long running')` returned an empty list — this alarm no longer exists. This matches the broader fact that the `Custom/segment-publisher` namespace was removed around 2026-06. Do not spend a tool call probing for `segment-publisher long running alam`; go straight to the direct-evidence path below.

Historical shortcut (kept for context, no longer usable): when that companion alarm was in `ALARM` with a datapoint overlapping the `VolumeReadIOPs` breach window, it was treated as strong evidence that the IOPS spike came from the known `segment-publisher` scheduled batch workload. The batch queries (`user_journey_sessions_*`, `event_intermediate_counts_*`) run in parallel across reader instances, producing the uniform ReadIOPS signature described above.

**Current direct-evidence path (use this instead)**: skip the companion-alarm check entirely and confirm the batch-workload hypothesis directly from:
1. Performance Insights `current_top_projects_by_load` / `top_sql` — dominant project + `users_*`/`event_intermediate_counts_*`/`device_*` table families on the readers.
2. Per-instance `ReadIOPS` (all readers ramping together, even if unevenly) + cluster `ReadLatency` staying under ~5ms + per-instance `CPUUtilization` staying under ~60% on the busiest instance.
3. The scope-recovery aggregate in `references/rds-volume-read-iops-scope-recovery-via-user-journey-aggregate.md` to name the dominant project/user-journey.

This three-point check (PI dominant project + healthy latency/CPU + scope aggregate) is sufficient on its own for `no_action` classification; the companion alarm was never required, just a convenient shortcut that is now gone.

See `references/segment-publisher-slow-eic-query-noise.md` § "Cross-reference: RDS VolumeReadIOPs correlation" for the reverse direction (also affected by the same namespace removal).
