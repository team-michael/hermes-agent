...
- Even `Period=3600`, `3/6` can flap repeatedly. CloudWatch reevaluates one-hour periods every minute, so all six period boundaries slide together; borderline values can repeatedly enter and leave the evaluation set and produce several `OK -> ALARM` notifications from one workload episode.
- Before changing the alarm, replay raw 1-minute metric data with the actual sliding-window semantics and compare candidate configurations against `describe_alarm_history(HistoryItemType='Action')`. A useful validation is that the simulated current configuration reproduces the actual notification count before trusting counterfactuals.
- Prefer persistence over simply raising/lowering by guess:
  - warning-only starting point: `<=25%`, 1h period, `4/6` when replay shows `3/6` still chatters; keep `3/6` if the observed distribution does not flap
  - critical/page: `<=20%`, 1h period, `3/6`, and only as a composite with latency/I/O/DBLoad deterioration
- Treat these as replay-backed candidates, not universal constants. In one seven-day Notifly replay, `<=25%, 3/6` reproduced 10 actual actions while `<=25%, 4/6` reduced them to 3 and `<=20%, 3/6` produced none.
