...
Reference for Notifly CloudWatch/AWS Chatbot Slack custom-action buttons, including the `Run query` pattern where Chatbot invokes Lambda and posts the returned text as a thread reply: `references/notifly-aws-chatbot-custom-actions.md`.
...
Reference for Notifly EMF metric dimension design, custom metric cost shape, and existing project_id-as-dimension vs property patterns in api-service/Kakao poller/webhook receiver: `references/notifly-emf-dimension-cost-and-usage.md`.
...
Reference for Notifly `rendering_failure` investigations where successful events are in Kinesis/Athena rather than CloudWatch app logs, including Kakao Alimtalk sender-platform compatibility checks: `references/notifly-rendering-failure-athena-vs-cloudwatch.md`.
Reference for Notifly project-statistics/data-pipeline capacity reviews where the user asks whether Athena queries will be slow or hit quota: `references/notifly-project-statistics-athena-glue-capacity-review.md`.
...
Reference for tracing Notifly `/authenticate` 400 requests with missing/blank credentials to source IP and User-Agent by correlating EMF and structured `error-response` logs: `references/notifly-authenticate-empty-credentials-source-ip-tracing.md`.
...
9. **When the user says alerts feel noisier, inspect alarm config drift vs. real flapping**
   ...
   - when recommending new alarm values, compute both:
     ...
     - for long-period M-of-N alarms such as `Period=3600, 3/6`, do not replay only fixed clock-hour bins: CloudWatch can reevaluate every minute with minute-offset rolling one-hour windows. Reconstruct six rolling 60-minute averages per evaluation minute and replay both ALARM entry (`M` breaches) and OK recovery (`N-M+1` non-breaches). This catches repeated `OK ↔ ALARM` boundary flaps that otherwise look like independent incidents.
     - for Aurora `AuroraOptimizedReadsCacheHitRatio`, validate whether the cluster-dimension metric is aggregating only Optimized Reads readers; compare metric dimension sets, reader count, and `SampleCount` before attributing the alarm to the writer. See `references/cloudwatch-dynamic-rds-alarms-aurora-optimized-reads-cache-hit-ratio.md`.
...
For Notifly `project_statistics` daily scheduled runs, also use `references/notifly-project-statistics-scheduled-run-verification.md`: top-level Step Functions can be `SUCCEEDED` while the workflow internally finalized as failed; match schedule output -> Glue run -> PG rows per project and classify `succeeded_loaded` / `started_stopped` / `not_started`.
...
2. Inspect execution history, not just `list-executions.status`. Look for internal failure/finalization states such as `FinalizeWorkflowRunFailed`, `group_error`, `failed_stage`, `TaskFailed`, and caught exceptions.
3. Check Glue job runs for the exact job and filter by `Arguments['--PROJECT_ID']` or equivalent tenant/job arguments; capture `JobRunState`, target-window args, and `ErrorMessage`.
4. If SFn and Glue look green, inspect the scheduler payload/S3 schedule file before concluding success. A common failure mode is **selection omission**: the ETL succeeded for the scheduled entities, but the target entity was absent from the schedule payload.
5. For per-project pipelines, produce a project-level matrix: scheduled? Glue run started? Glue state? output row count? This catches partial states where a Glue run was `STOPPED` but already committed some rows.
6. Interpret distinctly:
   - SFn/Glue failed -> true ETL failure.
   - SFn top-level succeeded but internal run finalized failed -> caught workflow failure; do not call it healthy.
   - target project was scheduled but has no Glue run -> orchestration stopped before submission.
   - target project has a stopped/failed Glue run -> partial-output risk; re-run with clear/delete first.
...
7. In the final answer, lead with the mechanism: “ETL 자체 실패” vs “성공했지만 스케줄 대상에서 빠짐” vs “스케줄됐지만 Glue 제출 전 중단” are different operational causes and require different remediations.
   - SFn/Glue succeeded but target project has no job run -> scheduler did not select project/window, or Map aborted before reaching that project.
   - target project has job run but target entity absent from schedule JSON -> scheduler candidate-resolution gap.
   - target entity present and Glue succeeded but output absent -> transform/load/output-store issue.
8. In the final answer, lead with the mechanism: “ETL 자체 실패”, “성공했지만 스케줄 대상에서 빠짐”, “스케줄에는 있었지만 Glue 동시성/Map abort로 실행 안 됨”, and “실행됐지만 적재 안 됨” are different operational causes and require different remediations.

Concrete Notifly project-statistics scheduled-run checklist: `references/notifly-project-statistics-scheduled-run-triage.md`. It includes KST target-date/window calculation, class101 prod table verification, and the `Glue.ConcurrentRunsExceededException` pitfall.

When hardening a Step Functions + Glue workflow after a concurrency incident, prefer the minimal safe PR shape:
- Lower effective nested-Map concurrency below the Glue job limit with headroom; do not simply raise `MaxConcurrentRuns` unless capacity/cost/DB pressure has been reviewed.
- Add `Retry` with exponential backoff directly on every `arn:aws:states:::glue:startJobRun.sync` task that can collide with scheduled/backfill traffic.
- Include Glue error names that Step Functions may surface: `Glue.ConcurrentRunsExceededException`, `Glue.ThrottlingException`, and the wrapper `Glue.AWSGlueException`.
- If workflow failures are caught/finalized for metadata or Slack notification, end the state machine with an explicit `Fail` state afterward so GitHub Actions/CloudWatch do not show a misleading green run.
- Keep “drain all Map items and aggregate failures” as a follow-up if the current PR is a minimal incident hardening; it is useful but larger than a quick safety patch.
