...
3. **Cloudflare Container app + rollout**
   - Resolve the current application id by app name on each deploy; preview app recreation can make a previously recorded id return `404`.
   - `GET /accounts/$ACCOUNT_ID/containers/applications/<app-id>` should show the expected configuration/image target.
   - `GET /accounts/$ACCOUNT_ID/containers/applications/<app-id>/rollouts` should show the latest rollout status, target image/version, placement progress, and health errors.
   - Container rollout is asynchronous after `wrangler deploy`; a GitHub job can succeed before serving has converged.
   - Do not use rollout `status=completed` or `current_configuration` as the only serving proof. Cloudflare metadata can temporarily report `completed` while `current_configuration.image` still names the prior image, even after the custom domain has begun serving the target image. Verify the live custom domain with an authenticated revision-bearing `/health` response or a cache-busted artifact/feature probe. When metadata and live responses disagree, report both and treat the live response as runtime truth rather than waiting indefinitely for metadata convergence.
4. **Runtime health**
   - Authenticated `/health` should return the expected `sha`; for frontend-only changes, also probe a changed static locale/asset key or the actual page/API behavior so a generic old health response cannot mask stale serving.
...
- `.github/workflows/cf_preview.yml` runs on PR `opened`, `synchronize`, `reopened`, and `ready_for_review` targeting `main` when non-test preview-relevant paths change.
- `detect-changes` emits booleans for `api_service`, `internal_api_service`, and `web_console`, but it must compute the PR's changed files from the PR merge-base / GitHub PR files list, not a raw `github.event.pull_request.base.sha..head.sha` diff. A base branch that advanced can make `git diff base.sha head.sha` include unrelated main-branch service changes and falsely deploy `api-service`/`internal-api-service`.
- `deploy-api-service`, `deploy-internal-api-service`, and `deploy-web-console` should each depend only on `detect-changes`; do **not** chain them through each other. This keeps preview deploys independent/parallel and prevents a skipped/failed upstream preview from gating an unrelated service.
- Do **not** force `internal-api-service` just because both `api-service` and `web-console` changed; each service preview should be selected by its own tracked PR changes or explicit deploy-all cases only.
- Do **not** globally block `api-service` or `internal-api-service` in `.github/workflows/cf_deploy.yml`: manual `workflow_dispatch` deploys must still be able to choose `api-service`, `internal-api-service`, and `web-console`.
- This is not an ECS stage/prod deploy, even though `environment: stage` is passed for preview runtime/build configuration.

Domain derivation for the current web-console-only preview:
- branch name comes from `github.head_ref || github.ref_name`, sanitized with `s/[^a-zA-Z0-9-]/-/g` and lowercased.
- web-console URL: `https://<sanitized-branch>-console.notifly.tech/auth/login`
...
If the user asks about a currently running deployment, optionally query the GitHub run/job to confirm the actual `head_branch` and whether the workflow file came from an older revision; but the domain formula is still branch-derived. For a high-level explanation, keep it terse: “PR 브랜치용 Cloudflare web-console preview 환경을 띄우는 액션; ECS 배포 아님.”
...
- Preview cleanup can silently leave a stale Container application when the workflow runs `wrangler containers list --json` with Wrangler 4.59.2: that version does not support `--json`, and a non-fatal fallback can make the cleanup job green after deleting only the Worker/Durable Object. The next deploy then fails with `application ... associated with a different durable object namespace`. Pin a version that supports JSON output (verified with `wrangler@4.113.0 containers list --help`), confirm the cleanup log says the named container was deleted, then rerun the deploy. Scope Cloudflare credentials to the actual cleanup step so package installation lifecycle scripts cannot access them.
