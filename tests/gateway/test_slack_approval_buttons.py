...
class TestSlackExecApproval:
    ...
    @pytest.mark.asyncio
    async def test_owner_only_policy_mentions_approver_and_hides_persistent_buttons(self):
        adapter = _make_adapter()
        mock_client = adapter._team_clients["T1"]
        mock_client.chat_postMessage = AsyncMock(return_value={"ts": "1234.5678"})
        ...
        with patch(
            ...
            return_value=frozenset({"U_OWNER"}),
        ):
            await adapter.send_exec_approval(
                chat_id="C1", command="git push origin HEAD", session_key="s"
            )
        ...
        assert "<@U_OWNER>" in kwargs["blocks"][0]["text"]["text"]
...
class TestSlackApprovalAction:
    ...
    @pytest.mark.asyncio
    async def test_non_approver_cannot_resolve_even_when_chat_is_open_to_everyone(self):
        adapter = _make_adapter()
        _attach_auth_runner(adapter)
        adapter._approval_resolved["1234.5678"] = False
        ...
        body = {
            "message": {"ts": "1234.5678", "blocks": []},
            "channel": {"id": "C1"},
            "user": {"name": "coworker", "id": "U_COWORKER"},
        }
        action = {
            "action_id": "hermes_approve_once",
            "value": "agent:main:slack:group:C1:1111",
        ...
        with (
            patch("tools.approval.can_user_resolve_approval", return_value=False),
