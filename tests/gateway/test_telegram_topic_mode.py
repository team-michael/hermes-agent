...
async def test_topic_root_command_checks_getme_capabilities_before_enabling(tmp_path, monkeypatch):
    ...
    # GitHub runners can execute this test before process monotonic time exceeds
    # the cooldown. A missing prior send must still count as sendable.
...
def test_lobby_reminder_is_debounced_per_chat(tmp_path, monkeypatch):
...
def test_capability_hint_is_debounced_per_chat(tmp_path, monkeypatch):
...
def test_topic_off_resets_debounce_counters(tmp_path, monkeypatch):
