...
class TestIncomingDocumentHandling:
    ...
    @pytest.mark.asyncio
    async def test_amazon_q_attachment_blocks_are_surfaced_without_actions(
        self, adapter
    ):
        """Amazon Q puts CloudWatch alarm facts in attachment Block Kit blocks."""
        alarm_name = "/aws/ecs/notifly-services-prod/web-console/sentry alert"
        event = self._make_event(
            text="",
            attachments=[
                {
                    "fallback": f"CloudWatch Alarm | {alarm_name}",
                    "blocks": [
                        {
                            "type": "section",
                            "text": {
                                ...
                                "text": (
                                    ...
                                    "ap-northeast-2 | Account: 702197142747"
                                ),
                            },
                        },
                        {
                            ...
                            "text": {
                                "type": "mrkdwn",
                                "text": "Threshold Crossed: 1 datapoint >= 1.0",
                            },
                        },
                        {
                            "type": "actions",
                            "elements": [
                                {
                                    "type": "button",
                                    "action_id": "CALLBACK_CW_DASHBOARD",
                                    "text": {"type": "plain_text", "text": "Query logs"},
                                    "value": "AWS_CLI_COMMAND_ACTION_SECRET",
                                }
                            ],
                        },
                        {
                            ...
                            "fields": [
                                ...
                                {
                                    "type": "mrkdwn",
                                    "text": "*Timestamp*\nMon, 20 Jul 2026 07:32:43 UTC",
                                },
                                {"type": "mrkdwn", "text": "*Alarm State*\nALARM"},
                                {
                                    "type": "mrkdwn",
                                    "text": f"*Metric Alarm Name*\n{alarm_name}",
                                },
                            ],
                        },
                    ],
        ...
        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.splitlines()[0] == (
            f"CloudWatch Alarm | {alarm_name} | "
            "ap-northeast-2 | Account: 702197142747"
        )
        assert alarm_name in msg_event.text
        assert "Account: 702197142747" in msg_event.text
        assert "Threshold Crossed" in msg_event.text
        assert "ConsoleErrors" in msg_event.text
        assert "Mon, 20 Jul 2026 07:32:43 UTC" in msg_event.text
        assert "Alarm State\nALARM" in msg_event.text
        assert "AWS_CLI_COMMAND_ACTION_SECRET" not in msg_event.text
        assert "CALLBACK_CW_DASHBOARD" not in msg_event.text

    @pytest.mark.asyncio
    async def test_attachment_blocks_are_deduplicated_and_bounded(self, adapter):
        block_attachment = {
            "blocks": [
                {
                    "type": "section",
        ...
        event = self._make_event(
            text="investigate",
            attachments=[block_attachment, block_attachment],
        )

        await adapter._handle_slack_message(event)

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.count("UNIQUE_ALARM_FACT") == 1
        assert "... [truncated]" in msg_event.text
        assert len(msg_event.text) < 6500
